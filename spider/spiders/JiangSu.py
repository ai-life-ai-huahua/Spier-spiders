...
import json
import datetime
#某一页可能解析编码出问题导致后面的爬取失败，直到后一页的url来了 才能爬取下一页的内容
...
class JiangsuSpider(scrapy.Spider):
    ...
    def __init__(self,i=0,env=None,*args,**kwargs):
        super(JiangsuSpider,self).__init__(*args,**kwargs)
        self.i = i
        self.env = env

    def start_requests(self):
            #板块每一页的url
        url = 'http://jsggzy.jszwfw.gov.cn/EpointWebBuilder_jsggzy/dataWebAction.action?cmd=getDataList&categoryNum=004001001&projectName=&projectCode=&pageSize=15&pageIndex={}'.format(
        self.i)
        print(url,'!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    ...
    def parse(self, response):
        if  json.loads(json.loads(response.body.decode('utf-8'))['custom'])['Table']:
            print(json.loads(json.loads(response.body.decode('utf-8'))['custom'])['Table'],'-------------')
            for d in json.loads(json.loads(response.body.decode('utf-8'))['custom'])['Table']:
                url1 = 'http://jsggzy.jszwfw.gov.cn/EpointWebBuilder_jsggzy/dataWebAction.action?cmd=getDetail&rowguid={}&categorynum=4001001'.format(d['rowguid'])
                print(url1, '~~~~~~~~~~~~~', self.i)
