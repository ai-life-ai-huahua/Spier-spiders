...
class ToubiaorenSpider(scrapy.Spider):
    name = 'toubiaoren'
    allowed_domains = ['jsggzy.jszwfw.gov.cn']
    # start_urls = ['http://http://jsggzy.jszwfw.gov.cn/']

    def __init__(self, i=1522, env=None, *args, **kwargs):
        super(ToubiaorenSpider, self).__init__(*args, **kwargs)
        self.i = i
        self.env = env

    def start_requests(self):
        # for i in range(1523):
            url = 'http://jsggzy.jszwfw.gov.cn/EpointWebBuilder_jsggzy/dataWebAction.action?cmd=getDataList&categoryNum=004001003&projectName=&projectCode=&pageSize=15&pageIndex={}'.format(
                self.i)
    ...
    def parse(self, response):
        if json.loads(json.loads(response.body.decode('utf-8'))['custom'])['Table']:
            for d in json.loads(json.loads(response.body.decode('utf-8'))['custom'])['Table']:
                url1 = 'http://jsggzy.jszwfw.gov.cn/EpointWebBuilder_jsggzy/dataWebAction.action?cmd=getDetail&rowguid={}&categorynum=4001003'.format(
                    d['rowguid'])
                print(url1,self.i)
                ...
                if res == None:
                    ...
                    try:
                        date = json.loads(res.group().split(':', 1)[1])
                        print(date['time'], type(date['time']))
            ...
            self.i +=1
            url = 'http://jsggzy.jszwfw.gov.cn/EpointWebBuilder_jsggzy/dataWebAction.action?cmd=getDataList&categoryNum=004001003&projectName=&projectCode=&pageSize=15&pageIndex={}'.format(self.i)
            yield scrapy.Request(url=url,callback=self.parse)
        else:
